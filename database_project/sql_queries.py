...
def insert_client(is_company: bool, contact: str, name: str, nip: int, address: str):
    with connection.cursor() as cursor:
        ...
        sql = "INSERT INTO database_project_clients (id, is_company, contact, name, nip, address) " \
              "VALUES (%s, %s, %s, %s, %s, %s)"
...
def insert_nutritionalvalues(protein: float, carbohydrate: float, carbohydrate_of_witch_sugars: float, salt: float,
                             fat: float, fat_of_witch_saturates: float, energy: float, product_id: int):
    with connection.cursor() as cursor:
        max_val_sql = "SELECT max(id) AS val FROM database_project_nutritionalvalues"
        cursor.execute(max_val_sql)
        max_val = cursor.fetchone()[0]
        sql = "INSERT INTO database_project_nutritionalvalues (id, protein, carbohydrate," \
              " carbohydrate_of_witch_sugars, salt, fat, fat_of_witch_saturates, energy, product_id) " \
              "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
        is_product_id_sql = "SELECT * FROM database_project_nutritionalvalues WHERE product_id = %s "
        ...
        if is_product_id is None:
            # when tabel is empty max_val is None
            try:
                val = (max_val + 1, protein, carbohydrate, carbohydrate_of_witch_sugars, salt, fat,
                       fat_of_witch_saturates, energy, product_id)
            except TypeError:
                val = (1, protein, carbohydrate, carbohydrate_of_witch_sugars, salt, fat,
                       fat_of_witch_saturates, energy, product_id)
            except KeyError:
...
def insert_rawmaterials(rmid: int, name: str, quantity_in_stock: int, unit: str, sid: int):
    with connection.cursor() as cursor:
        max_val_sql = "SELECT max(id) AS val FROM database_project_rawmaterials"
        cursor.execute(max_val_sql)
        max_val = cursor.fetchone()[0]
        sql = "INSERT INTO database_project_rawmaterials (id, rmid, name, quantity_in_stock, unit, sid) " \
              "VALUES (%s, %s, %s, %s, %s, %s)"
...
def insert_stock(poss: int, item_id: int, quantity: float, placer: str, expiration_date: int, is_product: bool):
    # expiration_date is the number of days the product is fit for consumption
    with connection.cursor() as cursor:
        max_val_sql = "SELECT max(id) AS val FROM database_project_stock"
        cursor.execute(max_val_sql)
        max_val = cursor.fetchone()[0]
        sql = "INSERT INTO database_project_stock (id, poss, item_id, quantity, placement_time, placer," \
        ...
        # when tabel is empty max_val is None
        try:
            val = (max_val + 1, poss, item_id, quantity, placement_time, placer, expiration_date, is_product)
        except TypeError:
            val = (1, poss, item_id, quantity, placement_time, placer, expiration_date, is_product)
...
def insert_supplier(sid: int, name: str, nip: int, contact: str, bank_account: int):
    with connection.cursor() as cursor:
        ...
        cursor.execute(max_val_sql)
        max_val = cursor.fetchone()[0]
        sql = "INSERT INTO database_project_suppliers (id, sid, name, nip, contact, bank_account) " \
              "VALUES (%s, %s, %s, %s, %s, %s)"
...
def insert_technology(name: str, production_time_h: float, recipe: str):
    with connection.cursor() as cursor:
        ...
        cursor.execute(max_val_sql)
        max_val = cursor.fetchone()[0]
        sql = "INSERT INTO database_project_technology (id, name, production_time_h, recipe) " \
              "VALUES (%s, %s, %s, %s)"
